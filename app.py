...
# --- SISTEMA DI LOGIN (FIX DOPPIO CLIC) ---
def check_password():
    if "role" not in st.session_state:
        ...
        if st.button("Accedi"):
            try:
                # Lettura fresca dal foglio Config per evitare bug di sincronizzazione
                ...
                st.error("⚠️ Errore nel foglio 'Config'. Verifica colonne 'Ruolo' e 'Password'.")
        return False
    return True
...
# --- FUNZIONE CALENDARIO (PRIORITÀ ARANCIONE) ---
    ...
    for week in cal:
        ...
        for i, day in enumerate(week):
            if day == 0: html += '<td></td>'
                ...
                if i == idx_riposo:
                    bg, tx = "#ffa500", "white"
                # PRIORITÀ 2: Manuale (Verde/Rosso)
                elif len(stato) > 0:
...
if menu == "📊 Dashboard Oggi":
    ...
    # Filtro: togli chi riposa oggi e chi è segnato NON Disponibile
    ...
    with t2:
        ...
        if not modello.empty:
            ...
            if st.button("Esegui Copia Massiva"):
                if len(dst_range) == 2:
                    d_dest = [str(dst_range[0] + timedelta(days=i)) for i in range((dst_range[1]-dst_range[0]).days + 1)]
                    righe = []
                    for g in d_dest:
